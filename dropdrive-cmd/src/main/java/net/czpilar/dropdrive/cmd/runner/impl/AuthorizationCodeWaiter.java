...
public class AuthorizationCodeWaiter {
    ...
    public Optional<String> getCode() {
        ...
        try {
            System.out.println("Waiting " + WAIT_TIME + " minutes for receiving authorization code to authorize the dropDrive application...");
            code = doWait();
        } catch (IOException | InterruptedException e) {
            throw new CommandLineException(e);
        } finally {
            if (code.isEmpty()) {
                System.out.println("dropDrive application was not authorized due to expired waiting time.");
            }
        }
        return code;
    }
    ...
    private Optional<String> doWait() throws IOException, InterruptedException {
        HttpServer server = HttpServer.create(new InetSocketAddress(dropDriveSetting.getRedirectUriPort()), 0);
        AuthorizationCodeWaiterHandler handler = new AuthorizationCodeWaiterHandler();
        server.createContext(dropDriveSetting.getRedirectUriContext(), handler);
        server.start();
        ...
        public void handle(HttpExchange exchange) throws IOException {
            if (exchange.getRequestMethod().equalsIgnoreCase("GET")) {
                ...
                String html = code.map(_ -> """
                        ...
                        <p>dropDrive application has been authorized. You can close this window.</p>
                        </body></html>"""
                ).orElse("""
                        <html><body style="font-family:sans-serif;text-align:center;padding:50px;">
